...
selected_month = st.sidebar.selectbox("월을 선택하세요", options=sorted(df['월'].unique()), fmt="%d월")
...
if filtered_df.empty:
    ...
    # 3. 요구사항에 따른 색상 반영 (찐빨강: #B22222, 연한 파란색: #87CEFA)
    ...
    ax.set_xlabel('연도 (Year)', fontsize=11, fontweight='bold', labelpad=10)
    ax.set_ylabel('기온 (℃)', fontsize=11, fontweight='bold', labelpad=10)
    ax.set_title(f"역대 {selected_month}월 {selected_day}일의 기온 변화 추세", fontsize=14, fontweight='bold', pad=15)
