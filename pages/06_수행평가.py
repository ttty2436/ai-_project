    ...
    try:
        ...
        if not df_merged.empty:
            ...
            fig_scatter = px.scatter(
                ...
                hover_name="country", title="Scatter",
                size=df_size, size_max=18
            )
            st.plotly_chart(fig_scatter, use_container_width=True)
            
            corr_value = df_merged[alc_col].corr(df_merged[hap_col])
            st.subheader("Data Analysis Conclusion")
            st.write("Correlation Coefficient:")
            st.code(str(round(corr_value, 2)))
            
            if corr_value > 0.3:
                st.info("Positive correlation detected. Higher consumption relates to higher happiness.")
            elif corr_value < -0.3:
                st.info("Negative correlation detected. Higher consumption relates to lower happiness.")
            else:
                st.info("No significant correlation detected between two datasets.")
        else:
            st.error("No matched data.")

    except FileNotFoundError:
        st.error("CSV files not found. Check your GitHub repository.")
